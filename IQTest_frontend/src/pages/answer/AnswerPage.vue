        ...
        <v-card-text>
          <div v-for="(item, idx) in question" :key="idx">
            ...
            <v-radio-group v-model="$data.userChoice[item.id]" row>
              <div v-for="(answer, index) in item.answer_choice" :key="index">
              <v-radio
                v-if="answer"
                :key="index"
                :value="`${index}`"
              >
              <template v-if="isImageChoice(item)" slot="label"><img  v-bind:src="getImage(item, answer)" width="50px"></template>
              <template  v-else slot="label">{{ answer }}</template>
              </v-radio>
              </div>
            </v-radio-group>
            <hr />
